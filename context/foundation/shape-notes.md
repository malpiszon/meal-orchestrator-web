...
checkpoint:
  ...
  gray_areas_resolved:
    ...
    - topic: "unmodified-plan approval timing"
      decision: "no approval/finalization step — next week's MO upload moves the prior (possibly unmodified) plan into history; no scheduler and no separate 'finalized' state, fits the no-additional-infra MVP constraint (see the 2026-09-23 revision below)"
