...
### Primary
- The end-to-end flow works: MO POSTs a recommendation to mo-web, the user (after activating via invitation) sees the recommendation with history context on a dashboard, can accept it or swap meals, and saves a final plan that becomes part of history for future weeks.
...
- If MO's POST to mo-web fails, MO's existing email delivery still succeeds — mo-web is additive and must never become a point of failure for MO's core function. (A failed save on mo-web's side is acceptable to require manual retry, per the "no additional infrastructure" constraint.)

## Functional Requirements

### Ingestion & Accounts
- FR-001: MO can submit a user's weekly recommendation and associated data to the mo-web API in JSON format. Priority: must-have
  > Socrates: Counter-argument considered: "the JSON schema may be too thin — MO's current per-module payload might not carry everything mo-web needs (e.g. a provider-side meal ID)." Resolution: kept; schema completeness is already tracked as an Open Question and must be confirmed before/during implementation planning.
...
A plan moves through three states, clarified during the FR Socrates round: **upcoming** (MO's newest recommendation, arrives Tue/Wed, week hasn't started, still editable/swappable) → **in-progress** ("current" in the everyday sense — the week the user is actually eating through, already locked in) → **history** (superseded once MO uploads the next week's recommendation). Ratings apply to the in-progress plan (you can't rate meals you haven't eaten yet); macro/nutritional summaries can apply to the in-progress plan, past history entries, or the upcoming plan (recalculated live as the upcoming plan is edited).

### Weekly Plan
- FR-007: User can view the upcoming week's recommended plan on a dashboard. Priority: must-have
  > Socrates: Counter-argument considered: "if MO's POST hasn't arrived yet, the dashboard has nothing to show — no defined empty state." Resolution: kept; dashboard shows an explicit "no upcoming plan yet" waiting state rather than blank/stale content. (This challenge surfaced the upcoming/in-progress/history terminology split — see "Plan lifecycle" above.)
...
- FR-009: User can swap a recommended meal in the upcoming plan for another meal available in that week's menu. Priority: must-have
  > Socrates: No counter-argument; stands as written. Context surfaced: MO always validates the JSON before proceeding, so mo-web receives a complete menu or nothing. "Full menu" = the user's subscribed meal-count (2 for breakfast+lunch, up to 5 for the full suite) × per-meal variants (currently 3, since MO supports one provider) — well-defined by MO's payload, not ambiguous.
- FR-010: User can save their final upcoming-week plan (as-is or with swaps); saving locks it in as that week's plan. Priority: must-have
...
### History
- FR-011: A plan (saved or not) is finalized into history once MO uploads the next week's recommendation. Priority: must-have
...
- FR-013: User can assign custom ratings to meals in the in-progress (currently-being-eaten) week's plan, independent of the provider's aggregate ratings. Priority: nice-to-have
...
- FR-016: mo-web can import prior choices from Meal Orchestrator's historical logs to backfill history predating mo-web. Priority: nice-to-have
  > Socrates: Counter-argument considered: "MO's debug artifacts aren't a stable data source — parsing them for backfill could be brittle." Resolution: kept as nice-to-have, but flagged as higher-risk/exploratory — feasibility depends on investigating the actual log format, and this may need reprioritizing once that's known.
...
#### Acceptance Criteria
- History annotations only appear on meals that actually recur in the user's recent history; meals with no history show no annotation.
...
- No additional infrastructure (e.g. a message queue) between MO and mo-web for the MVP — a failed API call from MO to mo-web is handled by manual retry, not automatic retry/queueing.
...
1. **Does MO's current per-module data payload contain everything mo-web needs (e.g. a provider-side meal ID), or does MO need a small payload extension?** — Owner: user. Blocks a precise FR-001 schema and affects FR-009 (menu-swap data) and FR-011 (finalization matching). Probably yes for the core case, but unconfirmed for edge details.
...
_(populated in Step 6 if stack-shaped content surfaces)_
