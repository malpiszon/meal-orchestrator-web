...
- **Secrets**: Runtime secrets (`SUPABASE_URL`, `SUPABASE_KEY`, and later the MO ingestion token and any Supabase service-role key needed for invites) live in Workers Secrets via `npx wrangler secret put <NAME>`. Only the account owner can read or rotate them in the dashboard, and each change creates a new deployed version. The CI deploy token (`CLOUDFLARE_API_TOKEN`, scoped to *Workers Scripts: Edit* on this account, plus *Workers KV Storage: Edit* only if the SESSION KV is kept) and `CLOUDFLARE_ACCOUNT_ID` live in GitHub Actions secrets. Local values go in `.dev.vars` (gitignored). To rotate, generate a new key in Supabase, `wrangler secret put`, then revoke the old key.
...
- [ ] **Create the cloud Supabase project.** It doesn't exist yet; only the local Docker stack is used. Choose an EU region (e.g. Frankfurt `eu-central-1`) to stay close to the Worker's EU traffic. Record the project ref here once it exists.
- [ ] **Configure Supabase Auth for production.** Set the Site URL to `https://mo-web.<subdomain>.workers.dev` (or the custom domain), and add the preview wildcard `https://*-mo-web.<subdomain>.workers.dev/**` to the redirect allowlist. Keep email confirmation **on** in production; it's only turned off for local development.
- [ ] **Configure custom SMTP via Resend in the production Supabase project** (Authentication → Emails → SMTP Settings: host `smtp.resend.com`, port `465`, user `resend`, password = a Resend API key). This reuses MO's existing Resend account; create a **separate API key** for mo-web so either key can be rotated or revoked on its own. Use a sender on a Resend-verified domain. Without custom SMTP, Supabase's built-in mailer is rate-limited and reportedly only delivers to the org's team members, so invites (FR-003) and password resets (FR-005) would not reach other users. Confirm the current limits when creating the project.
- [ ] **Set Worker secrets from the cloud project:** `npx wrangler secret put SUPABASE_URL` and `npx wrangler secret put SUPABASE_KEY`. Do this before or right after the first deploy. Because the vars are `optional: true`, a missing secret doesn't fail the deploy; auth just breaks silently.
- [ ] **Add GitHub repository secrets** `SUPABASE_URL` / `SUPABASE_KEY` (the `ci` job's build step reads them), plus `CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_ACCOUNT_ID` for the deploy job.
