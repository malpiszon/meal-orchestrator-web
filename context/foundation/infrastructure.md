...
- [x] ~~**Disable self sign-up in production Supabase.**~~ Done 2026-09-24. The PRD (Access Control, US-02) says invitations are the only way to get an account, but the starter's public `/auth/signup` was live. "Allow new users to sign up" is now off; a production sign-up returns `Signups not allowed for this instance`. The `/auth/signup` page and API route still exist and should be removed when FR-003 invitations are built. Local dev keeps sign-up enabled for `npm run smoke`.
- [ ] **Decide how to handle Supabase free-tier pausing** (projects pause after ~7 days idle). See the risk register. Now required by a PRD non-functional requirement (added 2026-09-24); how to meet it is still open. Leading option: a GitHub Actions cron job making a trivial Supabase read every ~3 days (note: GitHub disables scheduled workflows in public repos after 60 days without commits).
